...
class SosCoordinator {
  ...
  Future<SosDispatchResult> dispatch({
    ...
    // 3c. If both server and silent SMS failed, fall back to deep-links for
    // each contact to give the user a visible last-resort option.
    if (!serverOk && smsSent == 0) {
      await _deepLinkFallback(contacts, message, errors);
    }
  ...
  /// Deep-link fallback: opens WhatsApp or Telegram for each contact.
  /// WhatsApp: contacts with app == 'all' or 'whatsapp'
  /// Telegram:  contacts with app == 'all' or 'telegram'
  Future<void> _deepLinkFallback(
    ...
    for (final c in contacts) {
      ...
      if (c.app == 'all' || c.app == 'whatsapp') {
        try {
          final wa = Uri.parse('https://wa.me/$phoneDigits?text=$encoded');
          if (await canLaunchUrl(wa)) {
            await launchUrl(wa, mode: LaunchMode.externalApplication);
            continue; // WhatsApp opened — skip Telegram for this contact
          }
        } catch (e) {
          errors.add('whatsapp_${c.name}: $e');
        }
      }

      if (c.app == 'all' || c.app == 'telegram') {
        try {
          // t.me/+<phone> opens a Telegram chat if the number is registered
          final tg = Uri.parse('https://t.me/+$phoneDigits?text=$encoded');
          if (await canLaunchUrl(tg)) {
            await launchUrl(tg, mode: LaunchMode.externalApplication);
            continue;
          }
        } catch (e) {
          errors.add('telegram_${c.name}: $e');
        }
      }

      // Last resort: plain phone call / SMS deep-link
      try {
        final smsUri = Uri.parse('sms:${c.phone}?body=$encoded');
        if (await canLaunchUrl(smsUri)) {
          await launchUrl(smsUri, mode: LaunchMode.externalApplication);
        }
      } catch (e) {
        errors.add('sms_deeplink_${c.name}: $e');
      }
    }
  }
