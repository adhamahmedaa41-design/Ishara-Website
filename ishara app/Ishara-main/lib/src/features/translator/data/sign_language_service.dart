...
class SignLanguageService {
  ...
  static const String _karslManifestAssetPath = 'assets/models/manifest_karsl.json';
  static const String _defaultModelFile = 'asl_v6.tflite';
  static const String _defaultLabelFile = 'label_map_v6.json';

  /// Active vocabulary: false = V6 (12-class default), true = KArSL (89).
  /// Mutable so the UI can flip it via [setUseKarslVocabulary]; the next
  /// `loadModel()` call (after [unload]) will pick the new manifest.
  bool _useKarsl = false;
  // ignore: avoid_setters_without_getters
  set useKarsl(bool value) => _useKarsl = value;
  ...
  static const Duration _minProcessIntervalHighDpr = Duration(milliseconds: 125); // ~8 fps
  static const double _lowLightThreshold = 45.0;

  // Circuit-breaker thresholds
  static const int _cbFrameDropCount = 6;   // burst 1: drop this many frames
  static const int _cbCooldownMs     = 500;  // burst 2: pause this long
  static const int _cbResetAfter     = 30;   // successful frames before burst resets
  ...
  int _cbDropRemaining = 0;   // frames left to skip (burst 1)
  bool _cbInCooldown = false; // true during burst-2 cooldown
  int _successfulFrames = 0;  // count since last burst; resets burst after _cbResetAfter
  ...
  /// Called for transient circuit-breaker recovery notices that should
  /// auto-clear rather than persist as errors in the UI.
  void Function(String)? onRecovery;
  ...
  void _handleCameraImage(CameraImage image) {
    ...
    // Circuit breaker: burst-1 frame drop
    if (_cbDropRemaining > 0) {
      _cbDropRemaining -= 1;
      _droppedFramesTotal += 1;
      return;
    }

    // Circuit breaker: burst-2 cooldown (async, so we just gate here)
    if (_cbInCooldown) {
      _droppedFramesTotal += 1;
      return;
    }
    ...
    final now = DateTime.now();
    // Apply tighter FPS cap on high-DPR (dense-pixel) devices
    final interval = (image.width > 1080 || image.height > 1080)
        ? _minProcessIntervalHighDpr
        : _minProcessInterval;
    if (now.difference(_lastProcessedAt) < interval) {
      return;
    }
  ...
  Future<void> _processFrame(
    ...
    try {
      ...
      if (message.contains('failed precondition')) {
        ...
        _successfulFrames = 0;
        dev.log(
          'precondition burst=$_preconditionErrorBurst',
          name: 'translator',
        );
        await _recoverFromPreconditionFailure();
      } else {
        _preconditionErrorBurst = 0;
      ...
      // Reset burst counter after sustained successful run
      _successfulFrames += 1;
      if (_successfulFrames >= _cbResetAfter && _preconditionErrorBurst > 0) {
        dev.log('circuit breaker reset after $_successfulFrames clean frames', name: 'translator');
        _preconditionErrorBurst = 0;
        _successfulFrames = 0;
      }
    }
  ...
  Future<Map<String, dynamic>> _loadManifest() async {
    // Try the active vocabulary first; on miss, fall back to V6 so the
    // translator still loads if the KArSL bundle hasn't been shipped yet.
    final paths = _useKarsl
        ? <String>[_karslManifestAssetPath, _manifestAssetPath]
        : <String>[_manifestAssetPath];
  ...
  Future<void> _recoverFromPreconditionFailure() async {
    // Burst 1 + 2 are extremely common transient camera glitches on
    // Samsung / Xiaomi YUV pipelines. Recover silently — no UI noise.
    if (_preconditionErrorBurst == 1) {
      _cbDropRemaining = _cbFrameDropCount;
      dev.log(
        'circuit breaker burst-1: dropping $_cbFrameDropCount frames (silent)',
        name: 'translator',
      );
      return;
    }
    ...
    if (_preconditionErrorBurst == 2) {
      _cbInCooldown = true;
      dev.log(
        'circuit breaker burst-2: ${_cbCooldownMs}ms cooldown (silent)',
        name: 'translator',
      );
      await Future<void>.delayed(Duration(milliseconds: _cbCooldownMs));
      _cbInCooldown = false;
      return;
    }

    // Burst 3+: full detector restart (rate-limited to once per 2 s).
    // Still silent — the user shouldn't see "Model reset" banners unless
    // they ask via the explicit Reset button.
    final now = DateTime.now();
    ...
    dev.log(
      'circuit breaker burst-$_preconditionErrorBurst: full detector restart (silent)',
      name: 'translator',
    );
    ...
    try {
      ...
      // Hard failure — surface as an error since recovery itself broke.
      onError?.call('Recovery failed — try restarting the camera.');
    }
  }

  /// Public entry point for the manual "Reset model" button in the UI.
  Future<void> resetModel() async {
    dev.log('manual model reset requested', name: 'translator');
    _preconditionErrorBurst = 0;
    _cbDropRemaining = 0;
    _cbInCooldown = false;
    _successfulFrames = 0;
    _lastPoseDetectorResetAt = DateTime.fromMillisecondsSinceEpoch(0);
    ...
    try {
      ...
      resetSession();
      // Silent — the user pressed the Reset button themselves, no need to
      // narrate it back at them.
    } catch (_) {
      _poseDetector = previous;
      onError?.call('Reset failed — try restarting the camera.');
    }
  }
