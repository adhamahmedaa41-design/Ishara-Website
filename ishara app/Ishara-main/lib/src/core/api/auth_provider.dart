...
final apiClientProvider = Provider<ApiClient>((ref) {
  ...
  final savedUrlRaw = prefs.getString('server_url')?.trim() ?? '';

  // Ignore stale local/LAN URLs from older builds — those point at a dev PC
  // and aren't reachable on a standalone phone install.
  final savedUrl = _isLocalUrl(savedUrlRaw) ? '' : savedUrlRaw;

  // Resolution order: --dart-define > saved URL (non-local) > built-in
  // production default in ApiClient.
  final effectiveBaseUrl = envBaseUrl.trim().isNotEmpty
      ? envBaseUrl
