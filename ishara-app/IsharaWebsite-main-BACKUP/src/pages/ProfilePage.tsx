  ...
  const handleSave = async () => {
    ...
    try {
      if (avatarFile) {
        ...
        // Apply optimistically AND refresh from server to ensure consistency
        const newPic = avatarRes.user?.profilePic ?? avatarRes.avatar ?? (avatarRes as any).profilePic;
        if (newPic) updateUser({ profilePic: newPic });
      }
