...
export function SafetySection({ language }: SafetySectionProps) {
  ...
  const content = {
    en: {
      ...
      sosAlertTitle: 'SOS Alert System'
    },
    ar: {
      title: 'نظام السلامة',
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <ScrollReveal direction="left">
            <div className="bg-card p-8 rounded-3xl shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h3>{t.emergencyContactsTitle}</h3>
          ...
          <ScrollReveal direction="right">
            <div className="bg-card p-8 rounded-3xl shadow-2xl flex flex-col min-h-[520px]">
              <h3 className="mb-6">{t.sosAlertTitle}</h3>

              <div className="flex justify-center items-center flex-1 py-8">
              <motion.button
                onClick={triggerSOS}
                disabled={contacts.filter(c => c.email.includes('@')).length === 0 || sosActive}
                className={`relative w-64 h-64 rounded-full ${contacts.filter(c => c.email.includes('@')).length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                  }`}
                ...
                <div className="relative z-10 flex flex-col items-center justify-center h-full text-white">
                  {sosActive ? (
                    <>
                      ...
                      <span className="text-2xl font-bold">{t.sosActive}</span>
                    </>
                  ) : (
                    <>
                      <Shield className="w-20 h-20 mb-4" />
                      <span className="text-3xl font-bold">SOS</span>
                      <span className="text-sm mt-2 text-white/80">{t.testSOS}</span>
                    </>
                  )}
                </div>
              </motion.button>
