...
# ── REST endpoint (simpler than SocketIO for Flutter http calls) ────────────

# Plain-Python sessions keyed by `X-Session-Id` header. Each entry has the
# same shape as a SocketIO session.
rest_sessions = {}


def _rest_session(session_id: str):
    ...
    return rest_sessions[session_id]


@app.route('/predict', methods=['POST'])
def predict():
    """One-shot frame inference.

    Body (multipart/form-data preferred to avoid base64 overhead):
        file: JPEG bytes (key "file")
      OR JSON {'image': '<base64-jpeg>'}

    Header (optional):
        X-Session-Id: stable string per device — keeps the 30-frame ring
        buffer alive across requests.
    """
    sid = request.headers.get('X-Session-Id', 'default')
    session = _rest_session(sid)
    ...
    try:
        ...
        return {**status, 'word': None, 'confidence': 0.0,
                'sentence': ' '.join(session['sentence'])}
...
def reset_session():
    sid = request.headers.get('X-Session-Id', 'default')
    if sid in rest_sessions:
        rest_sessions[sid]['sequence'].clear()
        rest_sessions[sid]['sentence'].clear()
        rest_sessions[sid]['smoother'].reset()
    return {'status': 'reset'}
...
def main():
    ...
    # Find model files. New default location: ./models/ next to app.py
    # (the server-side weights live here, separate from the Flutter app's
    # assets bundle which only ships the YOLO TFLite).
    script_dir = Path(__file__).resolve().parent
    local_models = script_dir / 'models'
    assets_dir = script_dir.parent.parent / 'assets' / 'models'

    model_path = args.model or str(local_models / 'asl_v6.tflite')
    labels_path = args.labels or str(local_models / 'label_map_v6.json')

    # Backwards-compat fallback to the old assets path.
    if not Path(model_path).exists():
        alt = assets_dir / 'asl_v6.tflite'
        if alt.exists():
            model_path = str(alt)
    if not Path(labels_path).exists():
        alt = assets_dir / 'label_map_v6.json'
        if alt.exists():
            labels_path = str(alt)
