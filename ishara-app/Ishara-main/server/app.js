...
app.use(
  cors({
    origin(origin, callback) {
      ...
      // Always allow local development origins (any port, http or https).
      if (/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(origin)) {
        return callback(null, true);
      }

      // If no allowlist is configured, allow all to avoid accidental lockout.
      if (allowedOrigins.size === 0 || allowedOrigins.has(origin)) {
        return callback(null, true);
      }

      // Don't throw — that turns the preflight into a 500. Just deny.
...
// Make sure preflight OPTIONS requests are answered for every route, even
// ones protected by rate-limit/auth, BEFORE those middlewares run.
app.options("*", cors());
...
const limiter = rateLimit({
  ...
  legacyHeaders: false,
  // Never rate-limit CORS preflight — limiting OPTIONS turns the browser
  // request into a network error and looks like the server is down.
