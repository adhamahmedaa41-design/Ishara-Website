...
import '../data/currency_yolo_detector.dart';
import '../data/multi_object_detector.dart';
import '../data/object_detection_service.dart';
import '../data/yolo_object_detector.dart';
...
class VisionController extends StateNotifier<VisionState> {
  ...
  static const Duration _liveThrottle = Duration(milliseconds: 400);
  ...
  Future<void> processImage(
    ...
    try {
      if (state.selectedTool == VisionTool.currency) {
        await _processCurrency(imageFile, skipPersist: fromLiveCamera);
      } else if (state.selectedTool == VisionTool.objects) {
        await _processObjects(imageFile, skipPersist: fromLiveCamera);
      } else {
        await _processReadText(imageFile, skipPersist: fromLiveCamera);
      }
  ...
  Future<void> _processCurrency(XFile imageFile, {bool skipPersist = false}) async {
    // 1. Prefer the bundled YOLOv8 EGP detector — multi-detect bounding
    // boxes per banknote, sum to a real total, render Arabic display.
    final yolo = await CurrencyYoloDetector.create();
    if (yolo.isReady) {
      // 0.60 confidence floor + class-aware IoU NMS + label-whitelist
      // happens inside detect(); see CurrencyYoloDetector.detect's
      // docstring for the full pipeline.
      final raw = await yolo.detect(
        File(imageFile.path),
        minConfidence: 0.60,
      );
      // Multi-frame consensus (≥2 of last 3 frames) — kills the
      // "100 EGP read as 1 EGP for one frame" misclassifications.
      final detections = CurrencyYoloDetector.voteFrames(raw);
      if (detections.isNotEmpty) {
        final total = CurrencyYoloDetector.sumEgp(detections);
        final headline = CurrencyYoloDetector.formatTotalArabic(total);
        // Tally per denomination for the breakdown line.
        final tally = <String, int>{};
        for (final d in detections) {
          tally[d.arabic] = (tally[d.arabic] ?? 0) + 1;
        }
        // Drop the leading "1×" prefix when count == 1 (looked like a
        // standalone "1 EGP" tile in earlier builds). For ≥2 bills of the
        // same denomination, attach a trailing × badge instead.
        final breakdown = tally.entries
            .map((e) => e.value == 1 ? e.key : '${e.key} ×${e.value}')
            .join('  •  ');
        // Real bbox overlays so the user can sanity-check what got merged.
        // Coordinates are normalised to [0, 1]; the overlay renderer should
        // multiply by the displayed image's pixel size.
        final overlays = detections
            .map((d) => VisionOverlayItem(
                  label: '${d.arabic} (${(d.confidence * 100).toStringAsFixed(0)}%)',
        ...
        state = state.copyWith(
          isProcessing: false,
          recognizedText: '',
          currencySum: headline,
          currencyBreakdown: breakdown,
          overlayMessage: headline,
          objectDetections: const <DetectedObject>[],
          overlayItems: overlays,
          lastCurrencyConfidence: detections
              .map((d) => d.confidence)
    ...
    // 2. Fallback: OCR-keyword heuristic (legacy path).
  ...
  Future<void> _processReadText(XFile imageFile, {bool skipPersist = false}) async {
  ...
  Future<void> _processObjects(XFile imageFile, {bool skipPersist = false}) async {
    final file = File(imageFile.path);

    // Ensemble: run every bundled YOLO (COCO 80 + OIv7 601 + e-waste 77)
    // in parallel and merge with cross-model NMS. Falls back to single
    // YoloObjectDetector or ML-Kit if the multi bundle isn't present.
    List<DetectedObject> primary;
    final multi = await MultiObjectDetector.create();
    if (multi.isReady) {
      primary = await multi.detect(file, minConfidence: 0.40);
    } else {
      final yolo = await YoloObjectDetector.create();
      if (yolo.isReady) {
        primary = await yolo.detect(file, minConfidence: 0.40);
      } else {
        primary = await _objectDetector.detectObjects(file);
      }
    }
    final labelResults = await _imageLabeler.labelImage(file);

    final merged = _mergeDetections(primary, labelResults)
        .where((d) => d.confidence >= 0.40)
        .toList();
    final top5 = merged.take(5).toList(growable: false);
    final topLabel = top5.isNotEmpty ? top5.first.label : '';
  ...
  _CurrencyParseResult _parseCurrencySum(String raw) {
    ...
    final patterns = <double, RegExp>{
      ...
      1: RegExp(r'(?:\b1\b|جنيه\s*مصري|جنيه|ج\.م|egp|£e|pound)', caseSensitive: false),
