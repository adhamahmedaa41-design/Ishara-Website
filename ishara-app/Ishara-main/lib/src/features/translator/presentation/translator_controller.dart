...
class TranslatorController extends StateNotifier<TranslatorState> {
  ...
  /// Whether the next `loadModel()` should target the KArSL vocabulary
  /// (89 Arabic signs) instead of the default V6 (12 signs). Pushed in
  /// from the screen via [setUseKarslVocabulary].
  bool _useKarslVocabulary = false;

  void setUseKarslVocabulary(bool value) {
    if (_useKarslVocabulary == value) return;
    _useKarslVocabulary = value;
    _signService.useKarsl = value;
  }

  void _initSignService() {
    // The legacy pose-LSTM service is on-device only (TFLite/FFI). On web,
    // it can't load anything anyway — leave every callback as a no-op so
    // its recovery banners ("Adjusting stream…", "Model reset…") never
    // surface in the UI. Web users hit the Roboflow-Hosted YOLO live
    // screen instead.
    if (kIsWeb) return;
    ...
    // Recovery notices are operational ("the camera/pose detector hit a
    // transient hiccup and we're self-healing"). They should NEVER look
    // like user-facing errors — the pose-LSTM path triggers them on
    // every camera glitch. Just drop them on the floor; the actual model
    // continues running underneath.
    _signService.onRecovery = (message) {
      if (!mounted) return;
      // If a previous recovery message accidentally ended up in
      // state.error (e.g. before this fix shipped), wipe it now.
      if (state.error != null &&
          (state.error!.contains('resync') ||
              state.error!.contains('signing') ||
              state.error!.contains('Adjusting') ||
              state.error!.contains('Model reset'))) {
  ...
  String _mapSignServiceError(String error) {
    ...
    if (lower.contains('failed precondition')) {
      return 'Stream desync detected. Tap "Reset model" if this persists.';
    }
    return error;
  }
  ...
  Future<void> toggleCamera() async {
    // Web has no on-device path; the legacy service must never start. The
    // Communicate screen redirects to /translator/yolo-live (Roboflow
    // Hosted) before this method is ever called on web, but guarding here
    // keeps stale calls (hot-reload, route stacks) from re-triggering the
    // recovery loop.
    if (kIsWeb) return;
    if (state.isCameraActive) {
      _signService.stopStreaming();
    ...
    if (!state.isModelReady) {
      // Always read the current toggle just before loading so a recent
      // vocabulary change in Settings takes effect on the next session.
      _signService.useKarsl = _useKarslVocabulary;
