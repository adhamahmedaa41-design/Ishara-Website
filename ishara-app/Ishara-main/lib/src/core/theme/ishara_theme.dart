...
class IsharaSpacing {
  ...
  // Bold-direction layout rhythm
  static const double sectionGap = 36;
  static const double cardGap = 18;
  static const double heroBlock = 56;

  static const EdgeInsets page = EdgeInsets.all(md);
  static const EdgeInsets cardPadding = EdgeInsets.all(md);
  static const EdgeInsets pageX = EdgeInsets.symmetric(horizontal: 22);
  static const EdgeInsets heroPad = EdgeInsets.fromLTRB(22, 18, 22, 24);
}

/// Motion tokens — every animation in the bold design language pulls
/// from these so timings stay consistent across screens.
class IsharaMotion {
  ...
  // Curves chosen for confident, slightly under-damped motion
...
class IsharaColors {
  ...
  static List<BoxShadow> cardShadow({required bool dark}) => [
    BoxShadow(
      color: dark ? Colors.black.withValues(alpha: 0.35) : const Color(0x1A0F172A),
      blurRadius: dark ? 18 : 14,
      offset: const Offset(0, 8),
    ),
  ];

  /// Bold-direction elevated surface — used by IsharaSurface. Larger radius,
  /// softer shadow, optional teal-glow ring.
  static List<BoxShadow> elevatedShadow({required bool dark, bool glow = false}) => [
    BoxShadow(
      color: dark ? Colors.black.withValues(alpha: 0.45) : const Color(0x140F172A),
      blurRadius: dark ? 28 : 22,
      offset: const Offset(0, 14),
    ),
    if (glow)
      BoxShadow(
        color: (dark ? tealDark : tealLight).withValues(alpha: 0.18),
        blurRadius: 40,
        spreadRadius: -6,
  ...
  static const BorderRadius surfaceRadius =
...
BoxDecoration glassmorphismDecoration({required bool dark}) => BoxDecoration(
  color:
      dark
          ? IsharaColors.darkCard.withValues(alpha: 0.92)
          : IsharaColors.lightCard,
  borderRadius: IsharaColors.cardRadius,
  border: Border.all(
    color:
        dark
            ? IsharaColors.tealDark.withValues(alpha: 0.12)
...
WidgetStateProperty<Color?> _overlayFor(Color color) {
...
WidgetStateProperty<BorderSide?> _outlinedSideFor(
  Color color,
  Color disabled,
) {
  return WidgetStateProperty.resolveWith((states) {
    ...
    return BorderSide(color: color.withValues(alpha: 0.35));
  });
}
...
ThemeData buildIsharaLightTheme() {
  ...
  final disabledBg = IsharaColors.lightForeground.withValues(alpha: 0.12);
  final disabledFg = IsharaColors.lightForeground.withValues(alpha: 0.38);
  ...
  return base.copyWith(
    ...
    elevatedButtonTheme: ElevatedButtonThemeData(
      style: ButtonStyle(
        ...
        shadowColor: WidgetStateProperty.all(
          IsharaColors.tealLight.withValues(alpha: 0.35),
        ),
    ...
    chipTheme: base.chipTheme.copyWith(
      ...
      selectedColor: IsharaColors.tealLight.withValues(alpha: 0.12),
      side: BorderSide(color: IsharaColors.tealLight.withValues(alpha: 0.22)),
...
ThemeData buildIsharaDarkTheme() {
  ...
  final disabledBg = IsharaColors.darkForeground.withValues(alpha: 0.12);
  final disabledFg = IsharaColors.darkForeground.withValues(alpha: 0.38);
  ...
  return base.copyWith(
    ...
    elevatedButtonTheme: ElevatedButtonThemeData(
      style: ButtonStyle(
        ...
        shadowColor: WidgetStateProperty.all(
          IsharaColors.tealDark.withValues(alpha: 0.45),
        ),
        backgroundColor: WidgetStateProperty.resolveWith((states) {
          if (states.contains(WidgetState.disabled)) return disabledBg;
    ...
    chipTheme: base.chipTheme.copyWith(
      ...
      selectedColor: IsharaColors.tealDark.withValues(alpha: 0.2),
      side: BorderSide(color: IsharaColors.tealDark.withValues(alpha: 0.3)),
      shape: RoundedRectangleBorder(borderRadius: IsharaColors.pillRadius),
      labelStyle: textTheme.labelMedium,
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
    ),
