...
class DataService {
  ...
  // ─────────────────────────────────────────────────────────────────
  // LEARNING PROGRESS (per-user, synced to DB)
  // ─────────────────────────────────────────────────────────────────

  Future<DataResult<Map<String, dynamic>>> fetchLearningProgress() async {
    try {
      final response = await _api.get('/api/users/learning-progress');
      final data = _extractMap(response.data);
      return DataResult(success: true, message: 'Progress loaded', data: data);
    } on DioException catch (e) {
      return DataResult(success: false, message: _extractMessage(e));
    }
  }

  Future<DataResult<Map<String, dynamic>>> saveLearningProgress({
    required Map<String, Map<String, bool>> progress,
    required List<String> activityDates,
  }) async {
    try {
      final response = await _api.put(
        '/api/users/learning-progress',
        data: {
          'progress': progress,
      ...
      final data = _extractMap(response.data);
      return DataResult(success: true, message: 'Progress saved', data: data);
    } on DioException catch (e) {
      return DataResult(success: false, message: _extractMessage(e));
    }
  }
